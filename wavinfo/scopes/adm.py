...
__all__ = ('read', 'AudioDefinitionModel', 'ChannelEntry')


def read(axml_data: bytes, chna_data: bytes):
    header_fmt = "<HH"
    uid_fmt = "<H12s14s11sx"

    #: An :mod:`lxml.etree` of the ADM XML document
    axml = ET.parse(BytesIO(axml_data))
    ...
    return AudioDefinitionModel(axml=axml,
                                channel_entries=channel_uids)
...
class AudioDefinitionModel:
    ...
    def track_info(self, index) -> Optional[dict]:
        ...
        """
        channel_info = next((x for x in self.channel_entries
